import { dequeueUrl, enqueueUrls, markDone, markFailed, updateNextFetch } from '../db/frontier.js';
 ...
 */
export async function runWorker(workerId, shuttingDown, activeWorkers) {
    ...
    while (!shuttingDown.value) {
        ...
        if (url === null) {
            // Keep count incremented during the sleep so the orchestrator's
            // termination poll does not fire early when all workers are idle.
            await sleep(500);
            activeWorkers.count--;
            continue;
        }
