...
// robots-parser ships a type file that opens with a bare `declare module 'robots-parser';`
// (no body), which makes TypeScript see the module namespace rather than the callable
// default export. Cast to the actual runtime shape explicitly.
interface RobotsTxtRules {
  getCrawlDelay(ua?: string): number | undefined
  isAllowed(url: string, ua?: string): boolean | undefined
}
type RobotsParserFn = (url: string, content: string) => RobotsTxtRules
const robotsParser = robotsParserImport as unknown as RobotsParserFn
import { fetch } from 'undici'
import { getDomainInfo, saveDomainInfo, type DomainInfo } from '../db/frontier.js'
...
const parsedRobotsCache = new Map<string, RobotsTxtRules>()
 ...
 */
export async function ensureDomainInfo(domain: string): Promise<DomainInfo> {
 ...
 */
export function isUrlAllowed(url: string, domainInfo: DomainInfo): boolean {
  if (!domainInfo.isAllowed) return false
  if (domainInfo.robotsTxt === null) return true
 ...
 */
export function getNextFetchAt(_domain: string, crawlDelayMs: number): Date {
